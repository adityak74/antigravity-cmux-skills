...
```bash
osascript -e 'display notification "Build complete" with title "Antigravity" subtitle "Tests" sound name "Submarine"'
```
...
Add to `~/.gemini/antigravity-cli/settings.json` for automatic notifications:
    ...
    "Notification": [
      {
        "matcher": "idle_prompt",
        "hooks": [
          { "type": "command", "command": "cmux notify --title 'Antigravity' --body 'Waiting for input'" }
        ]
      },
      {
        "matcher": "permission_prompt",
        "hooks": [
          { "type": "command", "command": "cmux notify --title 'Antigravity' --subtitle 'Permission' --body 'Approval needed'" }
        ]
      }
    ]
  }
...
## Antigravity Built-in Hook

cmux provides a dedicated Antigravity hook command that reads JSON from stdin:

```bash
cmux hooks agy session-start    # (alias: active) — mark session as active
cmux hooks agy stop             # (alias: idle) — mark session as idle
cmux hooks agy notification     # (alias: notify) — forward notification
cmux hooks agy prompt-submit    # — handle prompt submission
```
