...
**Always launch agents in interactive mode** (`agy -i 'prompt'`, NOT `agy -p 'prompt'`). Interactive mode shows real-time streaming output — you can watch agents think, call tools, and produce results. Print mode (`-p`) buffers all output and shows nothing until the agent finishes. Agents launched interactively don't auto-exit — they stay at the prompt after completing the task. The orchestrator detects completion via `read-screen` and closes the pane with `close-surface`.
...
cmux send --surface $S3 "agy -i \"\$(cat scratchpad/agent-3-prompt.md)\"\n"
...
```bash
cmux send --surface $S1 "agy -i 'implement auth module. When done, save a summary of changes to scratchpad/agent-1-auth.md'\n"
```
...
Indicators that an Antigravity agent has finished:
...
cmux send --surface surface:4 "cmux wait-for auth-ready --timeout 600 && agy -i 'run integration tests'\n"
```
...
```bash
# Primary: read persisted output (use view_file tool in Antigravity)
...
cmux new-workspace --cwd /project/frontend --command "agy -i 'rebuild React components'\n"
cmux new-workspace --cwd /project/backend --command "agy -i 'optimize API endpoints'\n"
cmux new-workspace --cwd /project/infra --command "agy -i 'update Terraform configs'\n"
...
cmux send --surface surface:3 "agy -i 'retry: implement auth module'\n"
